...
var childrule = function( pass, rulename ) {
  return function(ctxt,cb) {
    ...
    for( var i = 0; i < pn.length; i++ ) {
      var p = pn[i]
      if( !pass(ctxt,p) ) {
        ctxt.failure = ctxt.util.genfail(ctxt,rulename,p,ctxt.util.formatloc(ctxt.loc))
        return cb(null,ctxt)
      }
    }
...
*/
  ...
  type$: function(ctxt,cb) {
    ...
    if( check ) {
      if( !check(ctxt.point) ) {
        ctxt.failure = ctxt.util.genfail(ctxt,'type$',ctxt.point,typename,ctxt.util.formatloc(ctxt.loc))
      }
    }
