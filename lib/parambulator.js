...
function basicfail() {
  ...
  if(!ctxt) {
    throw new Error('Parambulator: ctxt.util.fail: ctxt undefined')
  }

  var err = ctxt.util.generr(ctxt,code,ctxt.rulespec,ctxt.util.formatloc(ctxt.loc))
  return cb(err,ctxt)
}
...
function Parambulator( spec, pref ) {
  ...
  self.validate = function( args, cb ) {
    ...
    ctxt.util = {generr:generr,formatloc:formatloc,fail:basicfail}
    ...
    function execrule(ruleI, ctxt) {
      if( ruleI < rulesteps.length ) {
        ...
        //console.log(rulestep.name+' isstackop='+isstackop)
