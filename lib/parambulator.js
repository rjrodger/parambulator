...
var quantrule = function( foundok, rulename ) {
  return function(ctxt,cb) {
    ...
    if( !foundok(found) ) {
      ctxt.failure = ctxt.util.genfail(ctxt,rulename,''+pn,ctxt.util.formatparents(ctxt.parents))
      return cb(null,ctxt)
    }
...
var childrule = function( pass, rulename ) {
  return function(ctxt,cb) {
    ...
    for( var i = 0; i < pn.length; i++ ) {
      var p = pn[i]
      if( !pass(ctxt,p) ) {
        ctxt.failure = ctxt.util.genfail(ctxt,rulename,p,ctxt.util.formatparents(ctxt.parents))
        return cb(null,ctxt)
      }
    }
...
/*
  required$: function(ctxt,cb) {
    ...
    for( var i = 0; i < pn.length; i++ ) {
      var p = pn[i]
      if( !ctxt.point[p] ) {
        ctxt.failure = ctxt.util.genfail(ctxt,'required$',p,ctxt.util.formatparents(ctxt.parents))
        return cb(null,ctxt)
      }
    }
  ...
  notempty$: function(ctxt,cb) {
    var pn = ctxt.util.proplist(ctxt)

    for( var i = 0; i < pn.length; i++ ) {
      var p = pn[i]
      if( !ctxt.point[p] || '' == ctxt.point[p] ) {
        ctxt.failure = ctxt.util.genfail(ctxt,'required$',p,ctxt.util.formatparents(ctxt.parents))
        return cb(null,ctxt)
      }
    }
...
*/
  ...
  wild$: function(ctxt,cb) {
    ...
    if( value ) {
      if( !gex(ctxt.rulespec).on(value) ) {
        ctxt.failure = ctxt.util.genfail(ctxt,'wild$',value,ctxt.rulespec,ctxt.util.formatparents(ctxt.parents))
      }
    }
  ...
  eq$: function(ctxt,cb) {
    ...
    if( value ) {
      if( ctxt.rulespec !== value ) {
        ctxt.failure = ctxt.util.genfail(ctxt,'eq$',value,ctxt.rulespec,ctxt.util.formatparents(ctxt.parents))
      }
    }
  ...
  re$: function(ctxt,cb) {
    ...
    if( value ) {
      ...
      if( !re.exec(value) ) {
        ctxt.failure = ctxt.util.genfail(ctxt,'re$',value,re.toString(),ctxt.util.formatparents(ctxt.parents))
      }
    }
  ...
  type$: function(ctxt,cb) {
    var typename = ctxt.rulespec.toLowerCase()
    ...
    if( check ) {
      if( !check(ctxt.point) ) {
        ctxt.failure = ctxt.util.genfail(ctxt,'type$',ctxt.point,typename,ctxt.util.formatparents(ctxt.parents))
      }
    }
...
function basicfail() {
  ...
  ctxt.failure = ctxt.util.genfail(ctxt,code,ctxt.rulespec,ctxt.util.formatparents(ctxt.parents))
  return cb(null,ctxt)
}
...
function Parambulator( spec, pref ) {
  ...
  self.validate = function( args, cb ) {
    var ctxt = {point:args,msgs:msgs,log:[],parents:[],pointstack:[]}
    ctxt.util = {genfail:genfail,formatparents:formatparents,fail:basicfail,proplist:proplist}
